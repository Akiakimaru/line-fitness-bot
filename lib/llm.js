...
function normalizeHeaderToken(tok) {
  const t = tok.replace(/^\uFEFF/, "").trim().replace(/\s+/g, " ").toLowerCase();
  if (t === "week") return "week";
  if (t === "day") return "day";
  if (t === "kind") return "kind";
  if (t === "slot") return "slot";
  if (t === "text") return "text";
  if (["calories", "kcal", "calories(kcal)", "calories (kcal)"].includes(t)) return "calories";
  if (["p", "protein"].includes(t)) return "p";
  if (["f", "fat"].includes(t)) return "f";
  if (["c", "carb", "carbs"].includes(t)) return "c";
  if (["tips", "tip", "note", "notes"].includes(t)) return "tips";
  return t;
}
...
function cleanseCsv(raw) {
  ...
  const headerNorm = headerRaw.map(normalizeHeaderToken);
  const expected = ["week", "day", "kind", "slot", "text", "calories", "p", "f", "c", "tips"];
  const ok =
    headerNorm.length >= expected.length &&
    expected.every((h, i) => headerNorm[i] === h);

  if (!ok) {
    console.warn("[cleanseCsv] header mismatch:", headerRaw);
  ...
  const rows = body.filter((line) => {
    ...
    const h0 = normalizeHeaderToken(cols[0] || "");
    const h1 = normalizeHeaderToken(cols[1] || "");
    return !(h0 === "week" && h1 === "day");
  });

  if (rows.length !== 35) {
    console.warn(`[cleanseCsv] warn: expected 35 rows, got ${rows.length}`);
  }
...
async function generateNextWeekWithGPT(getWeekAndDayJST) {
  ...
  const exists = rows.some(
    (r) => String(r._rawData[idx.Week]).trim() === String(nextWeek)
  );
  ...
  const trainBrief =
    trainLogs.map((x) => `- ${x.Date} ${x.Slot}: ${x.Text}`).join("\n") ||
    "- 直近トレーニング記録なし";
  const mealBrief = mealLogs
    .slice(-7)
    .map((x) => `- ${x.Date} ${x.Slot}: ${x.Text}`)
    .join("\n");
...
- **Training の Text は必ず具体的な種目・回数や時間を含む**
- 前週に過負荷の部位は翌日休養/別部位に分散（胸/背/脚/肩/腕/休養ローテーション）
`;
  ...
  const { rows: filtered, delim } = cleanseCsv(raw);
