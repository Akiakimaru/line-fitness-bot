 ...
 * GPTを使用して食品名を標準化・簡略化
 */
async function standardizeFoodNameWithGPT(foodName) {
  try {
    console.log(`[standardizeFoodNameWithGPT] Standardizing: ${foodName}`);
    
    const prompt = `
以下の食品名を標準的な食品名に変換してください。
...
例：
- "カリフォルニア堅果デイリーナッツ1袋" → "ナッツ"
- "セブンイレブンとり五目おにぎり" → "おにぎり"
- "VALX EAA 13g" → "EAA"
- "木綿豆腐130g" → "木綿豆腐"
- "R1ドリンクタイプ112g" → "R1ドリンク"

標準化された食品名のみを出力してください（説明は不要）：
`;

    const response = await generateText(prompt);
    const standardizedName = response.trim();
    
    console.log(`[standardizeFoodNameWithGPT] ${foodName} → ${standardizedName}`);
    return standardizedName;
    
  } catch (error) {
    console.error(`[standardizeFoodNameWithGPT] Error for ${foodName}:`, error);
    return foodName; // エラー時は元の名前を返す
  }
}
...
class DynamicFoodDatabase {
   ...
   * 新しい食品データを学習キューに追加
   */
  addToLearningQueue(foodName, pfcData, confidence = 0.8) {
    if (confidence >= 0.7) { // 信頼度が高い場合のみ学習
      ...
      console.log(`[DynamicFoodDatabase] Added to learning queue: ${foodName} (confidence: ${confidence})`);
  ...
  async processLearningQueue() {
    ...
    try {
      while (this.learningQueue.length > 0) {
        ...
        await this.learnFood(item);
      }
    } catch (error) {
      console.error('[DynamicFoodDatabase] Learning error:', error);
    } finally {
      this.isProcessing = false;
    }
  }
   ...
   * 個別の食品データを学習
   */
  async learnFood(item) {
    try {
      const { foodName, pfcData, confidence } = item;
      
      // 既存データがある場合は信頼度を比較
      const existing = this.database[foodName];
      if (existing && existing.confidence >= confidence) {
        console.log(`[DynamicFoodDatabase] Skipping ${foodName} (existing confidence: ${existing.confidence})`);
        return;
      }
      
      // データベースに追加
      this.database[foodName] = {
        protein: pfcData.protein,
        fat: pfcData.fat,
        carbs: pfcData.carbs,
        calories: pfcData.calories,
        unit: "100g",
        confidence: confidence,
        learned_at: new Date().toISOString(),
        source: pfcData.source || "gpt_learning"
      };
      
      console.log(`[DynamicFoodDatabase] Learned: ${foodName} - P${pfcData.protein}g F${pfcData.fat}g C${pfcData.carbs}g (${pfcData.calories}kcal)`);
      
    } catch (error) {
      console.error(`[DynamicFoodDatabase] Error learning ${item.foodName}:`, error);
    }
  }
   ...
   */
  getStats() {
    const total = Object.keys(this.database).length;
    const learned = Object.values(this.database).filter(item => item.learned_at).length;
    const original = total - learned;
...
// グローバル動的データベース
const dynamicDB = new DynamicFoodDatabase();

/**
 * 食品名を標準化（GPT + ローカルマッピング）
 */
async function standardizeFoodName(foodName) {
  // 1. ローカルマッピングを試行
  const nameMap = {
    // 主食
    "ご飯": "白米",
  ...
  // 直接マッチング
  if (nameMap[foodName]) {
    return nameMap[foodName];
  }
  
  // 2. GPTで標準化（長い食品名の場合）
  if (foodName.length > 15) {
    try {
      const standardized = await standardizeFoodNameWithGPT(foodName);
      if (standardized && standardized !== foodName) {
        console.log(`[standardizeFoodName] GPT standardized: ${foodName} → ${standardized}`);
        return standardized;
      }
    } catch (error) {
      console.error(`[standardizeFoodName] GPT standardization failed for ${foodName}:`, error);
    }
  }
...
async function calculatePFC(parsedItems) {
  try {
    ...
    for (const item of parsedItems) {
      const foodName = await standardizeFoodName(item.name);
      let foodData = dynamicDB.getFoodData(foodName);
      ...
      if (foodData) {
        ...
        source = foodData.source || "dynamic_database";
        
        if (foodData.learned_at) {
          learnedItems++;
        }
        
        console.log(`[calculatePFC] Using dynamic database for: ${foodName}`);
      } else {
        // GPTで検索
        ...
        if (gptData) {
          ...
          // 動的データベースに学習
          dynamicDB.addToLearningQueue(foodName, gptData, confidence);
        } else {
    ...
    console.log(`[calculatePFC] Processed: ${processedItems}, GPT: ${gptItems}, Learned: ${learnedItems}, Unknown: ${unknownItems}`);
    ...
    const result = {
      ...
      method: gptItems > 0 ? "gpt+dynamic_db" : "dynamic_db",
      stats: {
