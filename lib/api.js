...
function calculateUserSummary(logs) {
  const { parseGymLogText } = require('./utils');
  let weights = [];
  ...
  for (const log of logs) {
    if (log.Kind === "Weight") {
      ...
      // 新形式: Meta.sets/minutes が存在する場合（構造化済み）
      if (meta.sets !== undefined) {
        gymSets += meta.sets || 0;
        gymMinutes += meta.minutes || 0;
      } else if (Array.isArray(meta.parsed)) {
        // 中間形式: Meta.parsed は存在するが sets/minutes がない
        ...
        gymSets += parsed.totalSets;
        gymMinutes += parsed.totalMinutes;
      }
