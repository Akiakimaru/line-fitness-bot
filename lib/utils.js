...
function verifyUserLink(uid, exp, sig) {
  ...
  console.log("[verifyUserLink] input:", { uid, exp, sig, now, secret: secret.substring(0, 8) + "..." });
  
  if (!uid || !exp || !sig) {
    console.log("[verifyUserLink] missing params");
    return false;
  }
  if (Number(exp) < now) {
    console.log("[verifyUserLink] expired:", { exp, now, diff: now - Number(exp) });
    return false;
  }
  const payload = `${uid}.${exp}`;
  const expect = crypto.createHmac("sha256", secret).update(payload).digest("hex");
  console.log("[verifyUserLink] signature check:", { payload, expect, received: sig });
  
  // constant-time compare
  try {
    const result = crypto.timingSafeEqual(Buffer.from(expect), Buffer.from(String(sig)));
    console.log("[verifyUserLink] result:", result);
    return result;
  } catch (e) {
    console.log("[verifyUserLink] error:", e);
    return false;
  }
}
