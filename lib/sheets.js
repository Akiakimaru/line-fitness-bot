...
function getServiceAccount() {
  ...
  if (!jsonStr) {
    throw new Error('GOOGLE_SERVICE_ACCOUNT_JSON environment variable is not set. Please check your .env file.');
  }
  try {
...
function getJwt() {
  ...
  // JWTにJSON文字列を直接渡す方法を使用
  try {
    const creds = JSON.parse(credsJson);
    console.log('[getJwt] Creating JWT for:', creds.client_email);
    ...
    console.error('[getJwt] Error creating JWT:', e.message);
    throw e;
  }
}
...
/**
 * 過去N週間分の買い出し計画を取得（最新順）
 */
async function getRecentShoppingPlans(userId, weeks = 4) {
  const jwt = getJwt();
  const sheetsApi = google.sheets({ version: "v4", auth: jwt });
  ...
  for (const row of rows) {
    ...
    // userId一致 & status=active & 期間が指定範囲内
    if (planUserId === userId && status === "active" && validUntil >= weeksAgo) {
      try {
        const planJson = JSON.parse(row[5]);
        plans.push({
          week: row[0],
          ...
          status,
          // 現在有効かどうかのフラグ
          isCurrent: validFrom <= now && now <= validUntil
        });
      } catch (e) {
        console.error(`[getRecentShoppingPlans] JSON parse error:`, e);
        continue;
      }
    }
  }
  
  // 新しい順にソート（validFrom降順）
  plans.sort((a, b) => new Date(b.validFrom) - new Date(a.validFrom));
  
  console.log(`[getRecentShoppingPlans] Found ${plans.length} plans for user ${userId} in past ${weeks} weeks`);
