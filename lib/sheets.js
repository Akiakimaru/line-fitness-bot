...
async function getAllUserIds() {
  ...
  // 列名の揺れ対策：UserId / userId / userid 等を全部吸収
...
async function archiveOldWeeksBatch() {
  ...
  const archiveTitle = `Archive_${new Date().toISOString().slice(0, 7).replace("-", "")}`;
  ...
  console.log(`[archiveOldWeeksBatch] moved ${oldRows.length} rows to ${archiveTitle}`);
}
