const { readRecentLogs, updateLogPFC } = require('./sheets');
...
async function analyzeHistoricalMeals(days = 30) {
  try {
    ...
    // 成功したPFC解析結果をスプレッドシートに保存
    let savedCount = 0;
    for (const result of results) {
      if (result.pfcData && result.confidenceScore) {
        try {
          const success = await updateLogPFC(result.logId, result.pfcData, result.confidenceScore);
          if (success) {
            savedCount++;
            console.log(`[analyzeHistoricalMeals] Saved PFC data for: ${result.logId}`);
          } else {
            console.warn(`[analyzeHistoricalMeals] Failed to save PFC data for: ${result.logId}`);
          }
        } catch (error) {
          console.error(`[analyzeHistoricalMeals] Error saving PFC data for ${result.logId}:`, error);
        }
      }
    }
    
    console.log(`[analyzeHistoricalMeals] Saved ${savedCount} PFC records to spreadsheet`);
