...
async function analyzeHistoricalMeals(days = 30) {
  try {
    ...
    const results = batchResults.map((result, index) => ({
      logId: `${logsToProcess[index].DateTime}_${logsToProcess[index].UserId}`,
      originalLog: logsToProcess[index],
      pfcData: result.pfcData,
      confidenceScore: result.pfcData ? 0.8 : null,
      error: result.error
    }));
